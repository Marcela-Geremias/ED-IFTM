...
struct Lista {
    ...
    bool verificarExistencia(int x) {
        No *aux = cabeca;
        while (aux != NULL) {
            if(aux -> valor == x) {
                return true;
    ...
    void inserirElementoInexistente(int x){
        if(verificarExistencia(x) == false){
            inserirFinal(x);
        }
    }
...
int main() {
    ...
    printf("\nExercicio 1\n");
    printf("Elemento existe na lista? %s\n", l.verificarExistencia(5) ? "true" : "false");
    ...
    printf("\nExercicio 3\n");
    l.inserirElementoInexistente(15);
    l.inserirElementoInexistente(24);
    l.imprimir();
